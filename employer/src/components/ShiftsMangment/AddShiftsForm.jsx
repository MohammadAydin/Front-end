...
const AddShiftsForm = ({ isFormOpen, setIsFormOpen }) => {
  ...
  const time = [
    { label: t("Shifts.from"), timeField: "fromTime" },
    { label: t("Shifts.to"), timeField: "toTime" },
  ];
  ...
  const addShiftMutation = useMutation({
    ...
    onSuccess: (data) => {
      toast.success(data.message || t("Shifts.shiftAddedSuccess"));
    ...
    onError: (error) => {
      toast.error(
        error?.response?.data?.message || t("Shifts.failedToAddShift")
      );
    },
  });
        ...
        >
          ...
          >
            ...
            <div className="p-6 space-y-6">
              ...
              {addShiftMutation.error && (
                <div className="bg-red-50 border border-red-200 rounded-lg p-3">
                  <p className="text-red-600 text-sm">
                    {addShiftMutation.error?.response?.data?.message ||
                      t("Shifts.errorOccurredAdding")}
                  </p>
                </div>
              )}
            </div>
            ...
            <div className="p-6 bg-gray-50 border-t border-gray-200">
              <SubmitButtons
                ...
                isLoading={addShiftMutation.isLoading}
              />
            </div>
          </form>
        </div>
